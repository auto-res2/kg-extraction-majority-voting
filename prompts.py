...
後の検証ステップで精度を高めるため、この段階では再現率（recall）を優先してください。"""
    elif mode == "cross_sentence":
        mode_instruction = """
重要: 複数文にまたがる関係に注目してください。異なる文に登場するエンティティ間の関係を特に注意して抽出してください。
できるだけ多くの関係を漏れなく抽出してください。確信度が低い場合でも、可能性がある関係は候補として含めてください。"""
    elif mode == "structural":
        mode_instruction = """
重要: PartOf/HasPart, MemberOf等の構造的関係に注目してください。組織の階層関係、地理的包含関係、作品の構成要素などを特に注意して抽出してください。
できるだけ多くの関係を漏れなく抽出してください。確信度が低い場合でも、可能性がある関係は候補として含めてください。"""
